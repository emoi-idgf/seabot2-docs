...
.. code-block:: cpp
   ...
   : Node("iridium_node"), sbd_(), log_state_()
   {
     ...
     if (const int success = sbd_.init(serial_port_name_, serial_baud_rate_);
   ...
   void IridiumNode::init_parameters()
   {
     ...
     delay_last_gnss_ = std::chrono::seconds(this->get_parameter_or("delay_last_gnss",
       delay_last_gnss_.count()));
     ...
     enable_iridium_sending_ = this->get_parameter_or("enable_iridium_sending",
   ...
   void IridiumNode::init_interfaces()
   {
     ...
     subscriber_internal_sensor_filter_ = this->create_subscription<seabot2_msgs::msg::Bme280Data>(
       ...
       std::bind(&IridiumNode::internal_sensor_callback, this, _1));
   ...
   void IridiumNode::process()
   {
       ...
       enable_iridium_sending_)
     {
       ...
       if (t - time_last_log_version_ > delay_last_log_version_) {
         const string log_sentence =
           log_state_.serialize_log_state(static_cast<long long>(round(this->now().seconds())));
     ...
     if (sbd_.get_indicator_service() == 1 &&
       (send_data_required || sbd_.get_ring_alert() || sbd_.get_waiting() > 0))
     {
       ...
       bool flush_mo = false;
       if (const int session_mo_result = sbd_.get_session_mo();
         session_mo_result >= 0 && session_mo_result <= 4)
       {
