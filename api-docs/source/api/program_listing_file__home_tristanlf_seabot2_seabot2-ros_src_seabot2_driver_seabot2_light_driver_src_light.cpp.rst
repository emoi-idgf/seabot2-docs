...
.. code-block:: cpp
   ...
   int Light::i2c_open()
   {
     ...
     if (file_ < 0) {
       RCLCPP_WARN(n_->get_logger(), "[Light_driver] Failed to open the I2C bus (%s) - %s",
     ...
     if (result < 0) {
       RCLCPP_WARN(n_->get_logger(),
         "[Light_driver] Failed to acquire bus access and/or talk to slave (0x%X) - %s", I2C_SLAVE,
         strerror(result));
       exit(1);
     }
   ...
   {
     if (i2c_smbus_write_i2c_block_data(file_, REGISTER_LIGHT_PATTERN, pattern.size(),
       pattern.data()) < 0)
     {
   ...
   void Light::set_flash_number(const unsigned int & nb_flash) const
   {
     ...
     for(unsigned int i = 0; i < std::min(nb_flash * 2, static_cast < unsigned int > (NB_PATTERN));
       i += 2)
     {
