...
.. code-block:: cpp
   ...
   void Kalman::set_new_piston_data(const double position, double set_point, const rclcpp::Time &stamp) {
       piston_position_ = position;
       piston_stamp_ = stamp;
       compute_kalman(false, true);
   }
   
   void Kalman::set_new_depth_data(const double depth, const double velocity, const rclcpp::Time &stamp) {
       fusion_depth_ = depth;
