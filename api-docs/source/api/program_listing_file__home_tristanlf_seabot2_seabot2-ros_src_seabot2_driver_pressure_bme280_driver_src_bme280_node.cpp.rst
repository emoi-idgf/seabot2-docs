...
.. code-block:: cpp
   ...
   // Sensor initialization
     ...
     if ((file = open(i2c_periph_.c_str(), O_RDWR)) < 0) {
       RCLCPP_WARN(this->get_logger(), "[Pressure_BME280] Failed to open the I2C bus (%s)",
         i2c_periph_.c_str());
       exit(1);
     }
     ...
     if (ioctl(file, I2C_SLAVE, dev_.dev_id) < 0) {
       RCLCPP_WARN(this->get_logger(),
         "[Pressure_BME280] Failed to acquire bus access and/or talk to slave (0x%X)", I2C_SLAVE);
       exit(1);
     }
   ...
   void Bme280Node::print_settings() const
   {
     ...
     RCLCPP_DEBUG(this->get_logger(), "[Pressure_BME280] settings.standby_time = %i",
       dev_.settings.standby_time);
   }
   ...
   {
     publisher_sensor_ = this->create_publisher<seabot2_msgs::msg::Bme280Data>("pressure_internal",
       10);
   }
