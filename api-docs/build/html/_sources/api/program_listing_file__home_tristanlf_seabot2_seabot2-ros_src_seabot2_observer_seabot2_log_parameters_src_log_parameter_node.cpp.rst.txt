...
.. code-block:: cpp
   ...
   void LogParameterNode::service_record(
   ...
   void LogParameterNode::init_interfaces()
   {
     ...
     service_log_parameters_ = this->create_service<std_srvs::srv::Trigger>("log_parameters",
                                                                               bind(
       &LogParameterNode::service_record, this, _1, _2, _3),
   ...
   std::vector<std::string> LogParameterNode::get_param_list(const std::string & node_name)
   {
     ...
     while (!client_list->wait_for_service(1s)) {
       if (!rclcpp::ok()) {
         RCLCPP_ERROR(this->get_logger(),
           "[log_parameter_node] Interrupted while waiting for the service. Exiting.");
       }
     ...
     if (const auto status = future.wait_for(3s); status == std::future_status::ready) {
       ...
       RCLCPP_WARN(get_logger(), "[log_parameter_node] Fail get response from param_value %s",
         node_name.c_str());
   ...
   void LogParameterNode::get_param_values(
   ...
   void LogParameterNode::record_parameters()
   {
     ...
     msg.value.type = 4;
     publisher_parameters_->publish(msg);
     RCLCPP_INFO(this->get_logger(), "[log_parameter_node] %s %s %s %i", msg.node_name.c_str(),
       msg.param_name.c_str(), msg.value.string_value.c_str(), msg.value.type);
