...
.. code-block:: cpp
   ...
   int Piston::i2c_open()
   {
     ...
     if (int result = ioctl(file_, I2C_SLAVE, i2c_addr_); result < 0) {
       RCLCPP_WARN(n_->get_logger(),
         "[Piston_driver] Failed to acquire bus access and/or talk to slave (0x%X) - %s",
   ...
   int Piston::get_all_data()
   {
       ...
       REGISTER_DATA_SIZE)
     {
       ...
       const __u16 measured_battery_voltage = static_cast<__u16>(buff[0x0A]) +
         (static_cast<__u16>(buff[0x0B]) << 8);
       const __u16 measured_motor_current = static_cast<__u16>(buff[0x0C]) +
         (static_cast<__u16>(buff[0x0D]) << 8);
   
       battery_voltage_ = static_cast<double>(measured_battery_voltage) * CONVERSION_BRIDGE *
         ((R2_ + R1_) / R1_);
